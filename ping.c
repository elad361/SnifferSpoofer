...
int main(int argc, char* argv[])
{
	...
	char data[IP_MAXPACKET] = "This is the ping.\n", destIP[] = "8.8.8.8", exitMsg[] = "**EXIT**", startWDmsg[] = "START", endWDmsg[] = "RECEIVED", respon[IP_MAXPACKET];
	int datalen = strlen(data) + 1;

	memcpy(destIP, argv[1], sizeof(argv[1]));
	...
	//run watchdog and create a socket
	char* args[2];
	args[0] = "./watchdog";
	args[1] = NULL;
	int status;
	...
	sleep(1); // wait for watchdog to run
	printf("Waiting for watchdog to run\n");

	int watchDogFd, connfd;
	struct sockaddr_in watchdogAdr, cli;
	watchDogFd = socket(AF_INET, SOCK_STREAM, 0);

	if (watchDogFd == -1) {
		perror("socket creation to watchdog failed...\n");
		exit(0);
	}
	else printf("\nSocket to watchdog successfully created..\n");

	bzero(&watchdogAdr, sizeof(watchdogAdr));

	watchdogAdr.sin_family = AF_INET;
	watchdogAdr.sin_addr.s_addr = inet_addr(WATCHDOG_IP);
	watchdogAdr.sin_port = htons(WATCHDOG_PORT);

	if (connect(watchDogFd, (struct sockaddr*) & watchdogAdr, sizeof(watchdogAdr)) == -1) {
		perror("connection with watchdog server failed...\n");
		exit(0);
	}
	else
		printf("connected to watchdog server..\n");

	//watchdog sends SIGUSR1 when time is out, so timeout will handle this signal
	...
	while (1)
	{
		...
		send(watchDogFd, startWDmsg, sizeof(startWDmsg), 0);

		// Get the ping response
		bzero(respon, IP_MAXPACKET);
		socklen_t len = sizeof(dest_in);
		ssize_t bytes_received = -1;
		while ((bytes_received = recvfrom(sock, respon, sizeof(respon), 0, (struct sockaddr*) & dest_in, &len)))
		{
			printf("bytes received: %d", (int)bytes_received);
			if (bytes_received > 0)
			{
				// Check the IP header
				struct iphdr* iphdr = (struct iphdr*)respon;
				struct icmphdr* icmphdr = (struct icmphdr*)(respon + (iphdr->ihl * 4));
				// printf("%ld bytes from %s\n", bytes_received, inet_ntoa(dest_in.sin_addr));
				// icmphdr->type
				printf("\nSuccessfuly received one packet with %d bytes : data length : %d , icmp header : %d , ip header : %d \n", (int)bytes_received, datalen, ICMP_HDRLEN, IP4_HDRLEN);

				break;
			}
		}

		send(watchDogFd, endWDmsg, sizeof(endWDmsg), 0);

		gettimeofday(&end, 0);

		char reply[IP_MAXPACKET];
		memcpy(reply, respon + ICMP_HDRLEN + IP4_HDRLEN, datalen);
		// printf("ICMP reply: %s \n", reply);
	...
	send(watchDogFd, exitMsg, sizeof(exitMsg), 0);
